...
import com.github.dellixou.delclientv3.gui.oldgui.elements.Element;
...
public class ElementComboBox extends Element {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (comboAnimationProgress > 0) {
			...
			DrawHelper.drawRoundedRectOutline(x + 4, startY - 2 + (ay - startY + 6), width - 8, ay - startY + 6, 3, 1.5f, ColorUtils.getClickGUIColor());
			...
			for (String sld : set.getOptions()) {
				...
				if (sld.equalsIgnoreCase(set.getValString())) {
					DrawHelper.drawRoundedRect(x+5, ay+12, width-10, 13, 3, new Color(ColorUtils.getClickGUIColor().getRed(), ColorUtils.getClickGUIColor().getGreen(), ColorUtils.getClickGUIColor().getBlue(), 110));
				}
